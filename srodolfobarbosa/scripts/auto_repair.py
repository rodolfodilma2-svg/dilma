...
AUTO_INSTALL = (
    "true" == str(os.environ.get("AUTO_INSTALL", "true")).lower()
)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--patch-files",
        nargs="*",
        help="Archivos patch a aplicar"
    )
    parser.add_argument(
        "--labels",
        default="auto-repair",
        help="Labels para o PR (vírgula separadas)"
    )
    parser.add_argument(
        "--assignees",
        default=None,
        help="Assignees para o PR"
    )
    ...
    if args.sandbox:
        ...
        if decision == "merge" and args.auto_apply:
            ...
            if args.open_pr:
                print(f"👀 Sandbox requiere revisión humana (confianza={confidence:.0%})")
                print("   Abriendo PR para revisión...")
                # TODO: abrir PR
                sys.exit(0)
            else:
                print(f"⚠ Sandbox requiere revisión, pero --open-pr no está activado")
                sys.exit(1)
